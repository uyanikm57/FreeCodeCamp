...
echo "$($PSQL "SELECT DISTINCT(name)  FROM teams t LEFT JOIN games g  ON t.team_id=g.winner_id LEFT JOIN games g2 ON t.team_id=g2.opponent_id WHERE (g.year='2014' AND g.round='Eighth-Final') OR (g2.year='2014' AND g2.round='Eighth-Final');")"
